...
def transmitData(index):
    ...
    while not FINISHED[index]:
        try:
            ...
            print(f"An error occurred in transmitData for Sender {index + 1}:")
            traceback.print_exc()
            FINISHED[index] = True
...
def checkBusy(index):
    ...
    try:
        ...
        print(f"An error occurred in checkBusy for sender {index + 1}:")
        traceback.print_exc()
        FINISHED[index] = True
...
def utilizeChannel():
    ...
    try:
        ...
        print("An error occurred in utilizeChannel:")
        traceback.print_exc()
    finally:
